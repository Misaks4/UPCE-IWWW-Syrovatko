...
<?php
        $conn = Connection::getPdoInstance();
    if ($user[0][3] == 1) {
        $stmt = $conn->prepare("SELECT * FROM table_objednavky ORDER BY id DESC");
    }else{
        $stmt = $conn->prepare("SELECT * FROM table_objednavky WHERE user = :user ORDER BY id DESC");
        $stmt->bindParam(':user', $_SESSION["loggedUserId"]);
    }
        $stmt->execute();

        $dataTable = new DataTable($stmt->fetchAll());
        $dataTable->addColumn("user", "Uživatel");
        $dataTable->addColumn("product", "Produkty");
        $dataTable->addColumn("count", "Počet");
